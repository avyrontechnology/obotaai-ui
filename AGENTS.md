...
- `src/app/*`: routes `agents/` (`new`, `[id]`, `[id]/configure`), `batches/`, `calls/`, `flows/` (Graphs + Workflows tabs via `?tab=workflows`), `graphs/[id]`, `workflows/[id]` (builders), `library/`, `playground/`, `settings/` (general/security/webhooks/notifications — numbers/team/billing/api-keys/integrations are standalone sidebar routes), `numbers/`, `team/`, `billing/`, `api-keys/`, `integrations/` + `error.tsx`, `loading.tsx`, `not-found.tsx`, `globals.css`. `/setup/**` are thin `redirect()` shims to `/agents/*`, and `/graphs` + `/workflows` list routes are thin `redirect()` shims to `/flows` — do not add logic there.
...
- UI: `src/components/{common,dashboard,layout,builders,playground,settings,batches,library,flows}`. No `src/components/ui/` checked in yet — shadcn registry (`components.json`).
