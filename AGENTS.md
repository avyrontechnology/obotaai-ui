...
- Deep links: `/playground?agent=:id`, `/playground?agent=:id&mode=live`, `/calls?agent=:id`, `/batches?agent=:id&new=1`. Read via `useSearchParams` inside a `Suspense` boundary; derive `override ?? param ?? default` — never `setState` in an `effect` (lint `react-hooks/set-state-in-effect` fails the build).
- Live voice: `LiveTalk` (`components/playground/live-talk.tsx`, PCM utils in `lib/audio.ts`) talks to `WS_BASE_URL/chat/v1/:id` with JSON frames (`init`/`audio` base64 16k PCM in; `ack`/`audio` 24k PCM / `text` / `mark` / `clear` out; echo marks, stop playback on `clear`). The capture worklet MUST connect through a muted gain to destination or the graph is optimized away and no mic audio is sent. `ChatTalk` (`chat-talk.tsx`) is text-only over the same socket (`text` in, transcript `text`+`role` out; s2s agents only — pipeline sockets ignore the text queue). Backend browser-leg s2s requires the `task_manager` default-IO normalization + async `DefaultOutputHandler.set_stream_sid` + transcript forwarding + `_s2s_text_loop`/`send_text` (all in `../voiceai`), and a valid `OPENAI_API_KEY` in backend `.env` (container reads it at start — recreate after changing).
